...
- PythonOperator는 콜러블 함수(Callable)에서 추가 인수를 제공하는 방법도 지원함
    ...
    ```
- Airflow는 태스크 간 데이터를 전달하는 2가지 방법이 존재
    - Airflow 메타스토어를 사용하여 태스크 간 결과를 쓰고 읽음. (XCom)
    - 영구적인 위치(디스크 또는 DB)에 태스크 결과를 기록함
- 피클(pickle)은 파이썬의 직렬화 프로토콜이며, 직렬화는 메모리의 개체를 나중에 다시 읽을 수 있도록 디스크에 저장할 수 있는 형식으로 변환하는 것
- 피클링이 불가능한 개체로는 DB 연결과 파일 핸들러가 존재
- 크기가 작은 오브젝트일 때, XCom을 이용한 피클링이 적합
- 큰 데이터를 태스크 간에 전송하기 위해서는 Airflow 외부에 데이터를 저장
- DAG에서 template_serachpath 인수를 설정하면, Jinja는 기본 경로와 추가된 경로를 함께 탐색함
- PostgresOperator는 Postgres와 통신하기 위해, 훅(hook)이라고 불리는 것을 인스턴스화 함
    - 인스턴스화된 훅은 연결 생성, Postgres에 쿼리를 전송하고 연결에 대한 종료 작업을 처리함
